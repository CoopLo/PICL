...
def get_model(model_name, config, finetune=False):
    if(model_name == "fno"):
        if(finetune):
            model = FNO1d(config['num_channels'], config['modes'], config['width'], config['initial_step'])#, dropout=config['finetune_dropout'])
        else: # No Dropout
            model = FNO1d(config['num_channels'], config['modes'], config['width'], config['initial_step'])#, dropout=config['dropout']) 
...
def get_data(f, config):
    ...
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=config['pretrain_batch_size'],
                                               num_workers=config['num_workers'], shuffle=True)#,
                                               #generator=torch.Generator(device='cuda'))
    val_loader = torch.utils.data.DataLoader(val_data, batch_size=config['pretrain_batch_size'],
                                             num_workers=config['num_workers'], shuffle=False)#,
                                             #generator=torch.Generator(device='cuda'))
    test_loader = torch.utils.data.DataLoader(test_data, batch_size=config['pretrain_batch_size'],
                                             num_workers=config['num_workers'], shuffle=False)#,
                                             #generator=torch.Generator(device='cuda'))
    ...
    for ep in tqdm(range(config['pretrain_epochs'])):
        ...
        for bn, (xx, ys, grid, tokens, t, target) in enumerate(train_loader):
            ...
            if(isinstance(model, FNO1d)):
                ...
                if(config['contrastive_loss'] in ['physics_informed', 'GCL', 'normalGCL']):
                ...
                if(config['contrastive_loss'] in ['physics_informed', 'GCL']):
                    dx = grid[0,:,1] - grid[0,:,0]
                    grid = torch.swapaxes(grid, 1, 2)
    ...
    for ep in tqdm(range(start_epoch, config['epochs'])):
        ...
        for bn, (xx, yy, grid) in enumerate(train_loader):
            ...
            if(isinstance(model, FNO1d)):
                ...
                im, loss = model.get_loss(x, yy, grid, loss_fn)
                if(im.isnan().any()):
                    #print("NANS IN WEIGHTS?: {}".format(model.parameters().isnan().any()))
